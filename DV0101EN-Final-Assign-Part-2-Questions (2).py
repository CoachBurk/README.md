...
app.layout = html.Div([
    ...
    html.Div(dcc.Dropdown(
             id='select-year',
            options=[{'label': str(year), 'value': str(year)} for year in range(1980, 2014)],
        value='1980'
        )),
...
def update_input_container(selected_statistics, selected_year):
...
@app.callback(
   Output(component_id='output-container', component_property='children'),
    [Input(component_id='dropdown-statistics', component_property='value'), Input(component_id='selected_year', component_property='values')])
...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
	...
	# Hint:Use unemployment_rate,Vehicle_Type and Automobile_Sales columns
        unemp_data = recession_data.groupby(['unemployment_rate','Vehicle_Type'])['sales'].mean().reset_index()
...
# Plot 2 Total Monthly Automobile sales using line chart.
  ...
  # Plot bar chart for average number of vehicles sold during the given year
    ...
    # Total Advertisement Expenditure for each vehicle using pie chart
        ...
        title='Total Advertising Expenditure for Each Vehicle {}'.format(input_year))
